        ...
        foreach ($investments as $investment) {
            ...
            foreach ($investment->activeProperties as $property) {
                $row = [
                    ...
                    //Rodzaj nieruchomości: lokal mieszkalny, dom jednorodzinny
                    ($property->type == 1 || $property->type == 5)
                    ...
                    //Cena m2 powierzchni użytkowej lokalu mieszkalnego / domu jednorodzinnego [zł]
                    (isset($property->area) && is_numeric($property->area) && $property->area > 0
                        && in_array($property->type, [1, 5]))
                        ? round(
                        (float)($property->highlighted == 1 ? $property->promotion_price : $property->price_brutto)
                        / (float)$property->area,
                        2
                    )
                        : 'X',
...
//                        : 'X',
    ...
    {
        $today = now()->format('Y-m-d');
        $localDir = public_path("datasets");
        $remoteDir = '/new_www/datasets/';


        $errors = [];

        Log::info('FTP: Start wysyłki plików', [
            'localDir' => $localDir,
            'remoteDir' => $remoteDir,
            'date' => $today,
        ]);

        // CSV z dzisiejszą datą
        $csvFiles = glob($localDir . "/*{$today}.csv");

        if (empty($csvFiles)) {
            $warning = 'FTP: Brak plików CSV do wysłania';
            Log::warning($warning, ['pattern' => "*{$today}.csv"]);
            $errors[] = $warning;
        }
        ...
        foreach ($csvFiles as $filePath) {
            ...
            try {
                ...
                Log::info('FTP: Wysłano plik CSV', [
                    'file' => $fileName,
                ]);
            } catch (\Throwable $e) {
                $errorMsg = "FTP: Błąd wysyłki pliku CSV {$fileName}: " . $e->getMessage();
                Log::error($errorMsg);
                $errors[] = $errorMsg;
            }
        }
        ...
        // Stałe pliki
        foreach (['developer.xml', 'developer.md5'] as $file) {
            $fullPath = $localDir . '/' . $file;

            if (!file_exists($fullPath)) {
                $warning = "FTP: Brak pliku {$file}";
                Log::warning($warning);
                $errors[] = $warning;
                continue;
            }
            ...
            try {
                ...
                Log::info('FTP: Wysłano plik', [
                    'file' => $file,
                ]);
            } catch (\Throwable $e) {
                $errorMsg = "FTP: Błąd wysyłki pliku {$file}: " . $e->getMessage();
                Log::error($errorMsg);
                $errors[] = $errorMsg;
            }
        }

        Log::info('FTP: Zakończono wysyłkę plików');

        // Wysyłka maila podsumowującego
        $toEmail = 'kacpersky@gmail.com'; // podmień na właściwy email
        $subject = 'Podsumowanie wysyłki FTP: '.$today;

        if (empty($errors)) {
            $body = "Wysyłka plików na FTP zakończyła się pomyślnie.";
        } else {
            $body = "Wysyłka plików na FTP zakończyła się z błędami:\n\n" . implode("\n", $errors);
        }

        Mail::raw($body, function ($message) use ($toEmail, $subject) {
            $message->from('jacek@4dl.pl', 'FTP Notifier');
            $message->to($toEmail)
                ->subject($subject);
        });
    }
    ...
    public function showAsTable(Investment $investment)
    {
        ...
        foreach ($investment->activeProperties as $property) {
            ...
            $row = [
                ...
                //Cena m2 powierzchni użytkowej lokalu mieszkalnego / domu jednorodzinnego [zł]
                (isset($property->area) && is_numeric($property->area) && $property->area > 0 && $property->type == 1)
                    ? round(
                    (float)($property->highlighted == 1 ? $property->promotion_price : $property->price_brutto)
                    / (float)$property->area,
                    2
